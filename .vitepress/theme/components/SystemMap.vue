...
const flows = [
  { text: 'RLAR', w: 74, at: 202, href: '/flows/rlar' },
  { text: 'Flame Chase', w: 124, at: 236, href: '/flows/flame-chase' },
  { text: 'Humanize 1', w: 120, at: 270, href: '/flows/humanize1' },
  { text: 'Ralph Loop', w: 116, at: 304, href: '/flows/#ralph-loop' },
  { text: 'Goal', w: 66, at: 338, href: '/flows/#goal' },
...
const nodes = [
  {
    ...
    h: 128,
    kicker: 'Backends',
    title: 'Ten coding-agent CLIs',
    lines: ['claude · codex · dsh · agy', 'grok · kimi · qwen · pi', 'opencode · mimo'],
    href: 'https://hmz.humanfia.ai/reference/agents',
  },
  {
    key: 'kda',
    stage: 3,
  ...
  },
  {
    ...
    kicker: 'The referee',
    title: 'flowbench',
    lines: ['Scores the loops against', 'each other. Ours included.'],
    href: '/projects/flowbench',
  },
]

const links = [
  // The CLIs into the runtime: the one solid line, because it is the one thing that is not a
  // loop -- credentials and turns go in, and nothing comes back out this way.
  { key: 'in', stage: 0, solid: true, d: `M 262 310 L ${CX - CORE_R - 14} 310`, dur: '2.4s' },
  // The flows, doing the work, in the two places we point them.
  { key: 'kda', stage: 2, d: 'M 641 258 C 704 232, 726 176, 782 168', dur: '3.1s' },
  { key: 'hoa', stage: 2, d: 'M 641 362 C 704 372, 730 378, 782 378', dur: '3.1s' },
  // What the work is worth, going to the referee.
  { key: 'kda-bench', stage: 3, d: 'M 902 226 C 916 470, 780 634, 612 632', dur: '3.6s' },
  { key: 'hoa-bench', stage: 3, d: 'M 902 440 C 908 552, 782 646, 612 644', dur: '3.6s' },
...
<template>
  <figure class="map" :class="{ still: !motion }">
    <div class="map-scroll">
     <div class="map-stage">
      <svg
        ...
        role="img"
        aria-label="The Humanfia system: coding-agent CLIs feed the humanize runtime, flows orbit it, KDA and HOA are what it is pointed at, and flowbench scores the flows and feeds the winner back"
      >
        <defs>
    ...
    <figcaption>
      ...
      The CLIs you already log into feed the runtime; the flows orbit it; the applications are
      where they are found out; and flowbench sends the score back into the flows. Hover any
      piece to hold it.
    </figcaption>
  </figure>
</template>
